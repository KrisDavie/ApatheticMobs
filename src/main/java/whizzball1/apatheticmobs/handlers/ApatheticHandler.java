...
public class ApatheticHandler {
        ...
        if (!world.isRemote)
            if (ent instanceof MonsterEntity) {
                ...
                if (!ApatheticConfig.RULES.revenge.get()) {
                    ...
                    // ((EntityLiving) ent).tasks.executingTaskEntries.removeIf(t->t.action
                    // instanceof EntityAIHurtByTarget);
                }
                if (e.getEntity().getType() == EntityType.ENDER_DRAGON) {
    ...
    public void addCap(AttachCapabilitiesEvent<Entity> e) {
        ...
        if (ApatheticConfig.RULES.revenge.get())
            if (e.getObject() instanceof LivingEntity)
                if (e.getObject().getEntityWorld() != null)
                    if (!world.isRemote) {
    ...
    public boolean doI(LivingEntity entity) {
        boolean yes;
        if (!entity.isNonBoss() && ApatheticConfig.BOSS.bossOption.get())
            return false;
        ...
        if (yes && ApatheticConfig.RULES.revenge.get()) {
            ...
            if (entity.getRevengeTarget() != null) {
                yes = false;
                if (capability != null)
                    capability.setVengeful(true, entity);
            } else if (capability != null)
                if (capability.isVengeful()) {
        ...
        if (yes && !ApatheticConfig.RULES.revenge.get() && entity.getRevengeTarget() != null)
    ...
    public boolean isCorrectPlayer(LivingEntity livingEntity) {
        if (livingEntity instanceof PlayerEntity) {
            ...
            if (ApatheticConfig.RULES.playerWhitelist.get()) {
                if (WhitelistData.get((ServerWorld) livingEntity.getEntityWorld()).playerSet
                        .contains(ep.getUniqueID())) {
                    return true;
                } else
                    return false;
            } else
    ...
    public boolean revengeOver(IRevengeCap capability, LivingEntity entity) {
        if (!ApatheticConfig.RULES.revengeTime.get())
            return false;
