...
public class CommandApatheticWhitelist {
	...
	public static void register(CommandDispatcher<CommandSource> dispatcher) {
		dispatcher.register(
				Commands.literal("apatheticwhitelist").requires(commandSource -> commandSource.hasPermissionLevel(2))
						.then(Commands.literal("get").executes(context -> {
							WhitelistData data = WhitelistData.get(context.getSource().getWorld());
							StringBuilder b = new StringBuilder();
							for (UUID i : data.playerSet) {
								b.append(context.getSource().getServer().getPlayerProfileCache().getProfileByUUID(i)
										.getName());
								b.append(", ");
							}
							b.reverse();
							...
							return 1;
								...
								.then(Commands.argument("username", StringArgumentType.word()).executes(context -> {
									...
									UUID id = context.getSource().getServer().getPlayerProfileCache()
											.getGameProfileForUsername(username).getId();
									WhitelistData data = WhitelistData.get(context.getSource().getWorld());
									if (!data.playerSet.contains(id)) {
										...
										context.getSource().asPlayer().sendMessage(
												new StringTextComponent("commands.apatheticmobs.aw.playerfound"), null);
									}
									return 1;
								})))
						.then(Commands.literal("remove")
								.then(Commands.argument("username", StringArgumentType.word()).executes(context -> {
									...
									if (data.playerSet.contains(id)) {
										...
										context.getSource().asPlayer().sendMessage(
												new StringTextComponent("commands.apatheticmobs.aw.playerfailed"),
												null);
									}
